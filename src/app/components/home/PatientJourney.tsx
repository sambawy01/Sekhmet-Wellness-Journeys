    ...
    <section className="bg-white py-24 overflow-hidden">
      <div className="container mx-auto px-6 max-w-[1440px]">
        ...
        <div className="relative">
          {/* Connecting Line */}
          <div className="hidden lg:block absolute top-6 left-0 w-full h-0.5 bg-[#FAF7F2]">
            <div 
              className="h-full bg-[#B5724A] animate-grow-width"
              style={{ width: '0%' }}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-8 lg:gap-4">
            {steps.map((step, i) => (
              ...
              >
                <div className="w-12 h-12 rounded-full bg-white border-2 border-[#B5724A] flex items-center justify-center text-[#B5724A] font-['Space_Mono'] font-bold mb-4 shadow-sm group-hover:bg-[#B5724A] group-hover:text-white transition-colors duration-300">
                  {i + 1}
                </div>
                
                <div className="mb-3 text-[#2C2825] group-hover:text-[#B5724A] transition-colors duration-300">
                  <step.icon className="w-6 h-6 mx-auto" strokeWidth={1.5} />
                </div>
                
                <h3 className="font-['DM_Sans'] font-bold text-[#2C2825] text-lg mb-2">{step.title}</h3>
